...
if not interfaces:
    st.error("No Wi-Fi interfaces found. Ensure Wi-Fi is enabled or run with appropriate permissions.")
    st.stop()

iface = interfaces[0]  # Use the first available Wi-Fi interface
...
def get_live_rssi_data():
    ...
    try:
        ...
        rssi_values = [net.signal for net in results]
        if rssi_values:
    ...
    while capturing and len(rssi_series) < 50:
        ...
        if len(rssi_series) == 50:
            ...
            prediction_placeholder.markdown(
                f"<h3 style='color:blue; text-align:center;'>Predicted Gesture: <b>{predicted_gesture}</b></h3>",
